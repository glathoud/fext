...
(function (global) {
    ...
    function get_test_arr_sorted_search()
    {
        return [
            ...
            , function sorted_search_local_test()
            {
                var sortedArray = [ 0, 0, 0, 0, 1, 1, 3, 4, 6, 6, 7, 8, 9, 10, 10, 10,
                                    11, 13, 14, 14, 15 ];
                assert( '"0###3" === sortedSearchLocal( v, 0 ).join("###")', sortedArray );
                assert( '"4###5" === sortedSearchLocal( v, 1 ).join("###")', sortedArray );
                assert( 'null === sortedSearchLocal( v, 2 )', sortedArray );
                assert( '"8###9" === sortedSearchLocal( v, 6 ).join("###")', sortedArray );
                assert( 'null === sortedSearchLocal( v, 12 ) ', sortedArray );
                assert( '"18###19" === sortedSearchLocal( v, 14 ).join("###")', sortedArray );
                assert( '"20###20" === sortedSearchLocal( v, 15 ).join("###")', sortedArray );
