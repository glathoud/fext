...
(function (global) {
    ...
    // solved.
    // 
    // Problem: you cannot use `mfunD` here! No easy debugging.
    // So this way is not only "evil", but shitty as well.
    var improveFirst = mfun( improveFirst )
    ,   improveLast  = mfun( improveFirst, improveLast )
    ;
    ...
    improveFirst = eval( '(' + improveFirst.getImpl() + ')' );
    improveLast  = eval( '(' + improveLast.getImpl() + ')' );
