...
/*xxx
    ...
    function isOdd_isEven_check( isOdd, isEven, preprocess )
    {
        preprocess  ||  (preprocess = function ( x ) { return x; });
        [-7, -1, 1, 3, 5, 37 ].map( preprocess ).every( isOdd )
            && [-8, -2, 0, 2, 4, 6, 48 ].map( preprocess ).every( isEven )
            ||
            null.bug;
    }
